...
from lambdas.src.process_image import LambdaSettings, handler
...
class TestProcessImage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        env["INPUT_FILE_SUFFIXES"] = json.dumps([".png", ".svg", ".webp"])
        ...
        self.file = (Path(__file__).parent / "assets" / "image.png").absolute()

    def produce_event_and_file(self, key):
        event = {
            "Records": [
                {
                    "awsRegion": "eu-west-1",
        ...
        sns_event = utils.convert_s3_event_to_sns_event(event)
        file = (
            (Path(__file__).parent / "assets" / key.split("/")[-1])
            .absolute()
            .as_posix()
        )
        return event, sns_event, str(file), key
    ...
    def test_handler(self):
        with moto.mock_s3(), moto.mock_sns(), patch(
            ...
            s3_event, sns_event, file, key = self.produce_event_and_file(
                "test/image.png"
            )
            utils_mock.return_value = file

            create_bucket(env["DESTINATION_BUCKET"])
            create_bucket(env["SRC_BUCKET"])
            upload_file(env["SRC_BUCKET"], key, file)
            out = create_sns_topic("dev-lmu-large-media-upload-updates")
            env["SNS_TOPIC_ARN"] = out

            handler(sns_event, None)

            self.assertTrue(check_file_exists(env["DESTINATION_BUCKET"], key))
            self.assertFalse(check_file_exists(env["SRC_BUCKET"], key))

            # Check the file has the correct content type
            s3 = boto3.client("s3", "eu-west-1")
            response = s3.head_object(Bucket=env["DESTINATION_BUCKET"], Key=key)
            self.assertEqual(response["Metadata"]["content-type"], "image/png")
            ...
            self.assertEqual(
                msg.output_file,
                f"s3://dev-lmu-media-output-bucket/test/image.png",
            )
            self.assertEqual(
                msg.input_file,
                f"s3://dev-lmu-media-input-bucket/test/image.png",
            )
            self.assertEqual(msg.subject, "COMPLETE")

    def test_handler_svg(self):
        with moto.mock_s3(), moto.mock_sns(), patch(
            "lambdas.src.mediainfo.utils.presign_url"
        ) as utils_mock, patch("lambdas.src.mediainfo.utils.send_sns") as send_sns:
            s3_event, sns_event, file, key = self.produce_event_and_file(
            ...
            s3 = boto3.client("s3", "eu-west-1")
            response = s3.head_object(Bucket=env["DESTINATION_BUCKET"], Key=key)
            self.assertEqual(response["Metadata"]["content-type"], "image/svg+xml")
            ...
            self.assertEqual(
                msg.output_file,
                f"s3://dev-lmu-media-output-bucket/test/example.svg",
            )
            self.assertEqual(
                msg.input_file,
                f"s3://dev-lmu-media-input-bucket/test/example.svg",
            )
            self.assertEqual(msg.subject, "COMPLETE")

    def test_handler_webp(self):
        with moto.mock_s3(), moto.mock_sns(), patch(
            "lambdas.src.mediainfo.utils.presign_url"
        ) as utils_mock, patch("lambdas.src.mediainfo.utils.send_sns") as send_sns:
            s3_event, sns_event, file, key = self.produce_event_and_file(
            ...
            s3 = boto3.client("s3", "eu-west-1")
            response = s3.head_object(Bucket=env["DESTINATION_BUCKET"], Key=key)
            self.assertEqual(response["Metadata"]["content-type"], "image/webp")
            ...
            self.assertEqual(
                msg.output_file,
                f"s3://dev-lmu-media-output-bucket/test/example.webp",
            )
            self.assertEqual(
                msg.input_file,
                f"s3://dev-lmu-media-input-bucket/test/example.webp",
            )
            self.assertEqual(msg.subject, "COMPLETE")
    ...
    def test_handler_exception(self):
        with moto.mock_s3(), moto.mock_sns():
            ...
            with patch("lambdas.src.process_image.utils") as mock:
